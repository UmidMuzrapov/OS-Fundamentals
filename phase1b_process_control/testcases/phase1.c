...
void release_blocked_processes(MyPCB *process, enum BlockReason block_reason) {
    ...
    while (cur != NULL) {
        if (cur->block_reason == block_reason) {
            add_to_queues(cur);
...
void wrapper(void) {
    ...
    USLOSS_Console("User function returned.\n");
    quit(status);
}
...
void dispatcher() {
    ...
    if (DEBUG) USLOSS_Console("Entered dispatcher.\n");
    ...
    // case 4 there is a process with higher priority
        ...
        remove_from_queues();
        //move the current process TERM or BLOCKED, it had to be caught earlier
    ...
    // case 5 there is another process with the same priority and the current process has run for o>= 80 milliseconds.
                   ...
                   (current_process->run_start_time - currentTime()) >= 80)) {
...
void zap(int pid) {
    ...
    if (pid == INIT_PROCESS_ID) {
        snprintf(str, sizeof(str), "ERROR: Tried to zap init.");
        print_error(str, ERROR_STATE);
    }

    if (pid == current_process->process_id) {
        snprintf(str, sizeof(str), "ERROR: Cannot zap yourself.");
        print_error(str, ERROR_STATE);
    }
    ...
    if (target_process->process_state == TERM) {
        snprintf(str, sizeof(str), "ERROR: Cannot zap a terminated process.");
        print_error(str, ERROR_STATE);
    }

    while (target_process->process_state != TERM){
        current_process->process_state = BLOCKED;
...
int unblockProc(int pid) {
    ...
    if (target_process->process_state != BLOCKED || target_process->block_reason == SELF_BLOCKED) {
